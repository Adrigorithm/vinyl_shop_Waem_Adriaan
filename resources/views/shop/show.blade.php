...
@section('main')
    ...
    <div class="row">
        <div class="col-sm-4 text-center">
            <img class="img-thumbnail" id="cover" src="/assets/vinyl.png" data-src="{{$record->cover}}" alt="{{$record->title}}">
            <p>
                <a href="#!" class="btn btn-sm  btn-block mt-3 {{$record->btnClass}} {{$record->stock==0 ? 'disabled' : ''}}">
...
@section('script_after')
    <script>
        ...
        $.getJSON('{{$record->recordUrl}}').done(function(data){
            console.log(data);
            $.each(data.media[0].tracks, function(key, value){
                let length = value.length;
                if(value.length === null){
                    length = value.recording["length"];
                }
                let row = `<tr><td>${value.position}</td><td>${value.title}</td><td>${vinylShop.to_mm_ss(length)}</td></tr>`
                $('tbody').append(row);
            });
        }).fail(function(error){
